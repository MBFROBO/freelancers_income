...
Описание назначения каждого пакета:

* **config/** — отвечает за конфигурацию всего приложения.
  * `config.py` — код для загрузки и работы с конфигами.
  * `config.yaml` — основной файл настроек (например, параметры моделей, БД, API-ключи).
* **core/** — ядро логики приложения.
  * `main.py` — основной модуль ядра, здесь реализованы ключевые процессы, обработка запросов или пайплайн запуска сервисов.
* **database/** — работа с базой данных.
  * `connections.py` —  подключение к базе PostgeSQL.
* **model/** — всё, что связано с моделями машинного обучения или AI.
  * `model.py` — реализация логики основной модели.
  * `context.py` — работа с контекстом.
  * `rag/` — здесь харнятся пре-промпт системные контексты.
* **main.py** (в корне app/) — точка входа, инициализация всех компонентов.
